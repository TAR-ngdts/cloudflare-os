...
import { prepareSource, validateIdentity, type SourceFileInput } from "../src/source";
...
describe("prepareSource", () => {
  it("derives sizes from content and reports the framework", () => {
    ...
    expect(prepared.totalBytes).toBe(byteLength(JSON.stringify({ devDependencies: { vite: "^7" } })) + 4 + 14);
  ...
  it("requires explicit acknowledgement of every review finding", () => {
    ...
    expect(() => prepareSource(input(files))).toThrow(/review_findings_require_acknowledgement: backend_code, missing_lockfile|review_findings_require_acknowledgement: missing_lockfile, backend_code/);
    expect(() => prepareSource(input(files, ["backend_code"]))).toThrow(/missing_lockfile/);
    expect(prepareSource(input(files, ["backend_code", "missing_lockfile"])).reviewFindings).toHaveLength(2);
  });
